...
class GRANMixtureBernoulli(nn.Module):
  ...
  def _sampling(self, B):
    ...
    with torch.no_grad():
        ...
        if self.is_sym:
          # Undirected network, make adjacency matrix symmetric
          A = torch.tril(A, diagonal=-1)
          A = A + A.transpose(1, 2)
        
        else:
          # Directed network, run again to make the other adjacency matrix triangle
          # The above code's just copy/pasted again, will make this better later
          A_upper = A[:]
          with torch.no_grad():
