...
from gran.dataset import *
from gran.utils.logger import get_logger
from gran.utils.train_helper import data_to_gpu, snapshot, load_model, EarlyStopper
from gran.utils.data_helper import *
from gran.utils.eval_helper import *
from gran.utils.dist_helper import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv
from gran.utils.vis_helper import draw_graph_list, draw_graph_list_separate
from gran.utils.data_parallel import DataParallel
...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    ### load graphs
    if config.dataset.name in ('FIRSTMM_DB', 'DD', 'grid', 'lobster'): # examples given with GRAN
      self.graphs = create_graphs(config.dataset.name, data_dir=config.dataset.data_path)
    else:
      self.graphs = create_graphs(config.dataset.name, data_dir=config.dataset.data_path, fname=config.dataset.file_name)
    ...
    config.num_nodes_pmf_train = '['+', '.join(str(x) for x in self.num_nodes_pmf_train)+']'
  ...
  def test(self):
    self.config.save_dir = os.path.join(self.config.exp_dir, self.config.exp_name)
    ...
    if self.config.test.is_test_ER:
      ...
      save_name = os.path.join(self.config.save_dir, 'A_pred.pkl')
      pickle.dump(A_pred, open(save_name, 'wb'))          
      graphs_gen = [get_graph(aa) for aa in A_pred]
    
    # Save the generated graphs
    save_name = os.path.join(self.config.save_dir, 'graphs_gen.pkl')
    pickle.dump(graphs_gen, open(save_name, 'wb'))
